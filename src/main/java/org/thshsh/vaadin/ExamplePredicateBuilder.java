 ...
 * @since 1.10
 */
public class ExamplePredicateBuilder {
	
	public static final Logger LOGGER = LoggerFactory.getLogger(ExamplePredicateBuilder.class);
	...
	static List<Predicate> getPredicates(String path, CriteriaBuilder cb, Path<?> from, ManagedType<?> type, Object value,
		...
		for (SingularAttribute attribute : type.getSingularAttributes()) {
			...
			LOGGER.debug("SingularAttribute: {}",attribute);
			...
			LOGGER.debug("optionalValue: {}",optionalValue);
			...
			LOGGER.debug("getPersistentAttributeType: {}",attribute.getPersistentAttributeType());
			LOGGER.debug("getType: {}",attribute.getType());
			LOGGER.debug("getBindableJavaType: {}",attribute.getBindableJavaType());
			LOGGER.debug("getBindableType: {}",attribute.getBindableType());
			...
			if (attribute.getJavaType().equals(String.class)) {
				...
				Predicate p = cb.equal(from.get(attribute), attributeValue);
				predicates.add(p);
			}
		}
