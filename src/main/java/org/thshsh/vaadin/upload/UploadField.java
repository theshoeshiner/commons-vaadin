...
public class UploadField extends CustomField<List<UploadFile>> {
	...
	public UploadField(Receiver b) {
		...
		LOGGER.debug("initial value: {}",this.getValue());
	...
	protected void handleUploadFile(String name) {
		...
		if (receiver instanceof MultiFileMemoryBuffer) {
			...
			fileData = buffer.getFileData(name);
			desc = buffer.getFileDescriptor(name);
		}
		...
		try {
			...
			UploadFile uf = new UploadFile(fileData.getFileName(), fileData.getMimeType(), data,desc);
			addFile(uf);
	...
	protected void addFile(UploadFile uf) {
		...
		rem.addClassName("remove");
		rem.addThemeVariants(ButtonVariant.LUMO_TERTIARY_INLINE);
		fileLayout.add(rem);
		rem.addClickListener(e -> removeFile(uf));
		
		/*if(maxFiles != null && maxFiles.equals(files.size())) {
			upload.getElement().setAttribute("max-files-reached","");
		}*/
	...
	protected List<UploadFile> generateModelValue() {
		LOGGER.debug("generateModelValue files: {}",files);
	...
	protected void setPresentationValue(List<UploadFile> newPresentationValue) {
		...
		LOGGER.debug("current value: {}",getValue());
		//remove all current files
		List.copyOf(files).forEach(this::removeFile);
		if(newPresentationValue!=null) newPresentationValue.forEach(this::addFile);
		this.setValue(files);
		
		LOGGER.debug("new value: {}",getValue());
	}
	...
	protected boolean valueEquals(List<UploadFile> value1, List<UploadFile> value2) {
		...
		LOGGER.info("valueEquals: {}",b);
		return b;
	}

	@Override
	public void setValue(List<UploadFile> value) {
		LOGGER.debug("setValue: {}", value);
		super.setValue(value);
		LOGGER.debug("setValue exit");
	}
	
	@Override
	protected void setModelValue(List<UploadFile> newModelValue, boolean fromClient) {
		LOGGER.debug("setModelValue: {} fromclient: {}", newModelValue, fromClient);
	...
	public List<UploadFile> getValue() {
		LOGGER.debug("getValue: {}",super.getValue());
